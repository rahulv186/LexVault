...
const CryptoItem: React.FC<CryptoItemProps> = ({ icon, label, value, isFuture = false }) => (
  <div className={cn(
...
export const CryptoDetails: React.FC<{ evidence: Evidence }> = ({ evidence }) => {
  return (
    <div className="glass-card p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <CryptoItem
          ...
          label="IPFS CID"
          value={(evidence as any).cid}
          isFuture={true}
        />
        <CryptoItem
          icon={<Database className="w-4 h-4" />}
          label="Blockchain TX"
          value={(evidence as any).txHash}
          isFuture={true}
        />
        <CryptoItem
          icon={<Clock className="w-4 h-4" />}
          label="Anchor Timestamp"
          value={(evidence as any).anchorTimestamp}
          isFuture={true}
        />
        <CryptoItem
          icon={<Database className="w-4 h-4" />}
          label="Block Number"
          value={(evidence as any).blockNumber?.toString()}
          isFuture={true}
        />
      </div>
    </div>
